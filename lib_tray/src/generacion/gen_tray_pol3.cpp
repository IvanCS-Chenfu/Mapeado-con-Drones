...
namespace lib_tray
{
    ...
    GenTrayPol3::GenTrayPol3(std::size_t N_ejes): N_ejes_(N_ejes), coeficientes_(N_ejes)
    {
        if (N_ejes_ == 0) 
        {
    ...
    CoefCubic GenTrayPol3::calcular_coeficientes(double q0, double qf, double v0, double vf, double tf) const
    {
        if (tf <= 0.0) 
        {
    ...
    void GenTrayPol3::comprobar_tamano_vector(const std::vector<double> & vector, const char * nombre_vector) const
    {
    ...
    void GenTrayPol3::comprobar_tamano_vector(const std::vector<bool> & vector, const char * nombre_vector) const
    {
        if (vector.size() != N_ejes_) 
        {
            throw std::invalid_argument(std::string("El vector '") + nombre_vector + "' debe tener tamaño N_ejes = " + std::to_string(N_ejes_));
