...
namespace lib_tray
{
    ...
    GenTrayVelTrap::GenTrayVelTrap(std::size_t N_ejes): N_ejes_(N_ejes), coeficientes_(N_ejes)
    {
        if (N_ejes_ == 0)
        {
    ...
    CoefTrapVel GenTrayVelTrap::calcular_coeficientes(double q0, double qf, double v0, double vf, double vmax, double t_a) const
    {
        CoefTrapVel c;
        ...
        if (vmax <= 0.0)
        {
            throw std::invalid_argument("La velocidad maxima vmax debe ser mayor que 0 para ejes no estacionarios");
        }
        ...
        if (x1 + x2 >= distancia)     // Triangular
        {
            ...
            c.v_max = std::sqrt(std::max(0.0, c.a*distancia + 0.5*(std::abs(v0)*std::abs(v0) + std::abs(vf)*std::abs(vf))));
    ...
    void GenTrayVelTrap::comprobar_tamano_vector(const std::vector<double> & vector, const char * nombre_vector) const
    {
        if (vector.size() != N_ejes_)
        {
            throw std::invalid_argument(std::string("El vector '") + nombre_vector + "' debe tener tamaño N_ejes = " + std::to_string(N_ejes_));
        }
    }

    void GenTrayVelTrap::comprobar_tamano_vector(const std::vector<bool> & vector, const char * nombre_vector) const
    {
        if (vector.size() != N_ejes_)
        {
            throw std::invalid_argument(std::string("El vector '") + nombre_vector + "' debe tener tamaño N_ejes = " + std::to_string(N_ejes_));
