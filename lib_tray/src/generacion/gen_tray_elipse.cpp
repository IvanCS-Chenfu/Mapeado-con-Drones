...
namespace lib_tray
{
    ...
    GenTrayElipse::GenTrayElipse(std::size_t N_ejes): N_ejes_(N_ejes)
    {
        if (N_ejes_ == 0)
        {
    ...
    double t_a)
    {
        ...
        coeficientes_ = calcular_coeficientes(posiciones_iniciales, centro_elipse, parametros_elipse, yaw_absoluto, t_a);
    }
    ...
    double t_a) const
    {
        ...
        {
            const double theta_anterior = c.theta0 + 2.0 * PI * static_cast<double>(i - 1) / static_cast<double>(N_TABLA);
            const double theta_actual   = c.theta0 + 2.0 * PI * static_cast<double>(i)     / static_cast<double>(N_TABLA);
    ...
    void GenTrayElipse::evaluar_trayectoria(const CoefElipse & c, double t, std::vector<std::array<double, 5>> & salida) const
    {
        constexpr double PI = 3.14159265358979323846;
    ...
    void GenTrayElipse::comprobar_tamano_vector(const std::vector<double> & vector, const char * nombre_vector, std::size_t tamano) const
    {
        if (vector.size() != tamano)
        {
            throw std::invalid_argument(std::string("El vector '") + nombre_vector + "' debe tener tamaño " + std::to_string(tamano));
        }
    }
