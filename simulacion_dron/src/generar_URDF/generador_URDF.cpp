...
public:
  ...
  : rclcpp::Node("generador_URDF")
  {
    ...
    this->declare_parameter<std::vector<double>>(
      "fisico.cuerpo.matriz_inercia", {1e-4, 1e-4, 1e-4,
        0.0, 0.0, 0.0});
    ...
    this->declare_parameter<std::vector<double>>(
      "fisico.brazos.matriz_inercia", {1e-5, 1e-5, 1e-5,
        0.0, 0.0, 0.0});
    ...
    this->declare_parameter<std::vector<double>>(
      "fisico.motores.matriz_inercia", {1e-5, 1e-5, 1e-5,
        0.0, 0.0, 0.0});
    ...
    this->declare_parameter<std::vector<double>>(
      "fisico.camara.matriz_inercia", {1e-6, 1e-6, 1e-6,
        0.0, 0.0, 0.0});
    ...
    fisico_cuerpo_matriz_inercia =
      this->get_parameter("fisico.cuerpo.matriz_inercia").as_double_array();
    ...
    fisico_brazos_matriz_inercia =
      this->get_parameter("fisico.brazos.matriz_inercia").as_double_array();
    ...
    fisico_motores_matriz_inercia =
      this->get_parameter("fisico.motores.matriz_inercia").as_double_array();
    ...
    fisico_camara_matriz_inercia =
      this->get_parameter("fisico.camara.matriz_inercia").as_double_array();

    actuadores_conversor_fuerza2torque =
      this->get_parameter("actuadores.conversor.fuerza2torque").as_double();

    sensores_ground_truth_publish_rate =
      this->get_parameter("sensores.ground_truth.publish_rate").as_double();
    sensores_camara_porcentaje_estereo =
      this->get_parameter("sensores.camara.porcentaje_estereo").as_double();
    sensores_camara_publish_rate = this->get_parameter("sensores.camara.publish_rate").as_double();
    sensores_camara_width = this->get_parameter("sensores.camara.width").as_int();
...
private:
  ...
  {
    std::string nombre_paquete = "simulacion_dron";                 // Nombre del paquete
    std::string ruta_xacro = "/urdf/dron_plugins.xacro";               // Path del .xacro

    std::string xacro_path = ament_index_cpp::get_package_share_directory(nombre_paquete) +
      ruta_xacro;

    // Añadimos argumentos al archvo .xacro
    std::ostringstream args;
    args << " fisico_cuerpo_dim:=" << "\"" << fisico_cuerpo_dim[0] << " " << fisico_cuerpo_dim[1] <<
      " " << fisico_cuerpo_dim[2] << "\""
         << " fisico_cuerpo_color:=" << fisico_cuerpo_color
         << " fisico_cuerpo_masa:=" << fisico_cuerpo_masa
         << " fisico_cuerpo_matriz_inercia:=" << "\"" << fisico_cuerpo_matriz_inercia[0] << " " <<
      fisico_cuerpo_matriz_inercia[1] << " " << fisico_cuerpo_matriz_inercia[2] << " " <<
      fisico_cuerpo_matriz_inercia[3] << " " << fisico_cuerpo_matriz_inercia[4] << " " <<
      ...
      "\""
         ...
         << " fisico_brazos_matriz_inercia:=" << "\"" << fisico_brazos_matriz_inercia[0] << " " <<
      fisico_brazos_matriz_inercia[1] << " " << fisico_brazos_matriz_inercia[2] << " " <<
      fisico_brazos_matriz_inercia[3] << " " << fisico_brazos_matriz_inercia[4] << " " <<
      fisico_brazos_matriz_inercia[5] << "\""
         << " fisico_motores_dim:=" << "\"" << fisico_motores_dim[0] << " " <<
      fisico_motores_dim[1] << "\""
         << " fisico_motores_color:=" << fisico_motores_color
         << " fisico_motores_masa:=" << fisico_motores_masa
         << " fisico_motores_matriz_inercia:=" << "\"" << fisico_motores_matriz_inercia[0] << " " <<
      fisico_motores_matriz_inercia[1] << " " << fisico_motores_matriz_inercia[2] << " " <<
      fisico_motores_matriz_inercia[3] << " " << fisico_motores_matriz_inercia[4] << " " <<
      fisico_motores_matriz_inercia[5] << "\""
         << " fisico_camara_dim:=" << "\"" << fisico_camara_dim[0] << " " << fisico_camara_dim[1] <<
      " " << fisico_camara_dim[2] << "\""
         << " fisico_camara_color:=" << fisico_camara_color
         << " fisico_camara_masa:=" << fisico_camara_masa
         << " fisico_camara_matriz_inercia:=" << "\"" << fisico_camara_matriz_inercia[0] << " " <<
      fisico_camara_matriz_inercia[1] << " " << fisico_camara_matriz_inercia[2] << " " <<
      fisico_camara_matriz_inercia[3] << " " << fisico_camara_matriz_inercia[4] << " " <<
      fisico_camara_matriz_inercia[5] << "\""
