...
Fase 5: CONSEGUIDA; 5H integracion, 5I estabilizacion y 5J cierre 2/2
4A: CONSEGUIDA
4B: CONSEGUIDA
4C: CONSEGUIDA
4D: CONSEGUIDA
...
Fase 6 migrada: 6A PARCIAL (scheduler FIFO/IDs/STOP generacional y CTest 8/8); 6B/6C PARCIALES (asignacion workflow e interfaces de aceptacion inmediata compiladas); 6E/6G PARCIALES (evidencia KF reversible y materializacion incremental, CTest 9/9). La 786 valida en 6G/6H claims U reversibles por fuente/KF: dos captures `VIEW_ADVANCE` activaron 18 secciones, con vecinos continuos y rebanadas espaciales. 6I baja la inflacion adicional a un voxel, conserva reservas visibles y explica sus STOPs causalmente; 6H/6I siguen PARCIALES por relevo, progresion sostenida y cambios navegables UNKNOWN no resueltos. Los estados anteriores se conservan solo como historial legacy.
Migracion 6N: el barrido lateral de fachada, fallback sobre tres candidatos y validacion atomica de ambas capturas estan implementados; builds y tests dirigidos son correctos. 746 valida que el riesgo al 65 % conserva tracking, pero el buffer depth queda congelado en el frame 1367 tras recibos invalidos. Sus cuatro puntos dan confianza 0.059 y se reintentan 485 veces sin cooldown. Faltan frescura/soporte depth y validar integracion FREE, D*, movimiento y coverage.
Rendimiento transversal 2026-09-21: Fase 6 efectiva apagada ya evita crear y
calcular F6N; la mascara fisica y el protocolo de perdida ORB tienen gates
independientes. La prueba GT de dos drones completa seis goals, ejecuta ocho
optimizaciones con barrera y reduce la cola primaria maxima de 35 a 1. Una
revision visual posterior determina que el `-270°` se ejecuto como `+90°` y
que no hubo observacion del fiducial 3 ni reanclaje final. La repeticion v3 lo
resuelve con tres giros `-90°` y un cruce hasta `y=2`: KFs 79-80 observan el
objeto 3 y crean el primer ancla directa del nuevo epoch, sin requerir F3Q.
Quedan aplazados los costes internos de F6N cuando este habilitado.
...
Punto de entrada siguiente: acordar la correccion de 6N para no reutilizar una
captura depth rechazada sin evidencia nueva y aplicar backoff tras fallos. No
bajar simplemente la confianza para aceptar una observacion de cuatro puntos.
