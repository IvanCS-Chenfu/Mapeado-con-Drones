...
class Clase_Servicio_Accion : public rclcpp::Node
{
    ...
    private:
        ...
        std_msgs::msg::Float64MultiArray eval_cubic(const coef_cubic &c, double t, bool flag_angulo)
        {
        ...
        void execute(const std::shared_ptr<GoalHandleTrayAction> goal_handle)
        {
            ...
            if (!eliminado)
            {
                ...
                bool flag_relativo = static_cast<double>(goal->relativo);
                double xf, yf, zf, yawf;
                bool flag_angulo = false;

                if (flag_relativo)
                {
                    // Si es realtivo, la posición querida es la actual + target
                    ...
                    zf   = z0 + goal->target_pose.pose.position.z;
                    yawf = yaw0 + goal->target_pose.pose.orientation.z;

                    // Si yawf no está normalizado, la trayectoria obtenida creará problemas en el error de "control_calcular_fuerzas.cpp"
                ...
                else
                {
                    ...
                    zf   = goal->target_pose.pose.position.z;
                    yawf = goal->target_pose.pose.orientation.z;
                }
                ...
                while (rclcpp::ok() && !eliminado && t <= t_total) 
                {
                    ...
                    if (!goal_handle->is_canceling())
                    {
                        ...
                        std_msgs::msg::Float64MultiArray msg_x = eval_cubic(cx, t, false);
                        std_msgs::msg::Float64MultiArray msg_y = eval_cubic(cy, t, false);
                        std_msgs::msg::Float64MultiArray msg_z = eval_cubic(cz, t, false);
                        std_msgs::msg::Float64MultiArray msg_yaw = eval_cubic(cyaw, t, flag_angulo);
