...
bool Sim3Solver::ComputeSim3(Eigen::Matrix3f &P1, Eigen::Matrix3f &P2)
{
    ...
    if(!Pr1.allFinite() || !Pr2.allFinite() || !O1.allFinite() || !O2.allFinite())
        return false;
    ...
    if(!std::isfinite(vec_norm) || !std::isfinite(static_cast<double>(evec(0,maxIndex))))
        return false;
    ...
    {
        const double ang=atan2(vec_norm,evec(0,maxIndex));
        if(!std::isfinite(ang))
            return false;
    ...
    if(!mbFixScale)
    {
        ...
        if(!std::isfinite(nom) || !std::isfinite(cvnom))
            return false;
    ...
    if(!std::isfinite(static_cast<double>(ms12i)))
        return false;
