...
class Clase_Subscriber(Node):
    def __init__(self):
        ...
        self.volume = o3d.pipelines.integration.ScalableTSDFVolume(
            voxel_length=0.01,   # 1 cm por voxel
            sdf_trunc=0.04,      # truncación TSDF
            color_type=o3d.pipelines.integration.TSDFVolumeColorType.RGB8)
        
        # Datos TSDF
        self.T_cuerpo_mapa = np.eye(4, dtype=np.float64)
    ...
    def pose_cb(self, msg):
        ...
        self.T_cuerpo_mapa[:3, :3] = self.quat_to_rot(q.x, q.y, q.z, q.w)
        self.T_cuerpo_mapa[:3, 3] = np.array([p.x, p.y, p.z], dtype=np.float64)
    ...
    def depth2pc(self, rgb_img, depth_img):
        ...
        if not (rgb_img is None or depth_img is None or self.fx is None or self.fy is None or self.cx is None or self.cy is None):
            ...
            intrinsic = o3d.camera.PinholeCameraIntrinsic(width=rgb_img.shape[1],height=rgb_img.shape[0],fx=self.fx,fy=self.fy,cx=self.cx,cy=self.cy)
            
            T_camara_cuerpo = np.array([[0, 0, 1,  self.camara2cuerpo[0]],
                                        [-1, 0, 0,  self.camara2cuerpo[2]],
                                        [0,-1, 0, -self.camara2cuerpo[1]],
                                        [0, 0, 0,  1]])
            
            extrinsic = np.linalg.inv(self.T_cuerpo_mapa @ T_camara_cuerpo)
    ...
    def mapa_SGBM(self):
        ...
        if (self.img_der is not None) and (self.img_izq is not None) and (self.estado_accion == 4) and not (self.flag_publicado):
            ...
            if len(self.buffer_disparidad) >= self.n_promediado:
                ...
                self.depth2pc(self.img_izq, profundidad_media)
