...
class GlobalICPMap(Node):
    def __init__(self):
        ...
        #######
        # Mapa probabilístico por vóxeles
        self.voxel_map = {}
        self.voxel_size_map = self.voxel_size
        
        # Evidencia positiva / negativa
        self.hit_gain = 1.0
        self.miss_penalty = 0.70
        
        # Publicación y borrado
        self.score_min_publish = 1.0
        self.score_delete_threshold = -0.5
        
        # Penalizar solo si la nueva pose está cerca de la última vista que confirmó el voxel
        self.radio_actualizacion = 3.0
        self.radio_cercania_vista = 1.0
        #######
    ...
    def voxel_key(self, point):
        return tuple(np.floor(point / self.voxel_size_map).astype(int))
    #######


    #######
    def voxel_center(self, key):
        return (np.array(key, dtype=np.float64) + 0.5) * self.voxel_size_map
    #######


    #######
    def extraer_keys_observadas(self, cloud):
        points = np.asarray(cloud.points)
        keys_observadas = set()

        for p in points:
            keys_observadas.add(self.voxel_key(p))

        return keys_observadas
    #######


    #######
    def anadir_hits_voxel_map(self, cloud, sensor_pos):
        points = np.asarray(cloud.points)
        ...
        # Evidencia positiva: los puntos vistos ahora mismo se muestran desde la primera observación
        for p, c in zip(points, colors):
            key = self.voxel_key(p)
            keys_observadas.add(key)
            ...
            if key not in self.voxel_map:
                self.voxel_map[key] = {
                    ...
                    "sum_rgb": c.copy(),
                    "count_hits": 1,
                    "last_seen_pos": sensor_pos.copy()
                }
            else:
                self.voxel_map[key]["score"] += self.hit_gain
                self.voxel_map[key]["sum_xyz"] += p
                self.voxel_map[key]["sum_rgb"] += c
                self.voxel_map[key]["count_hits"] += 1
                self.voxel_map[key]["last_seen_pos"] = sensor_pos.copy()
    ...
    #######
    def penalizar_misses_voxel_map(self, keys_observadas, sensor_pos):
        keys_borrar = []

        # Evidencia negativa: si una vista cercana no vuelve a confirmar un voxel local, baja su confianza
        for key, voxel in self.voxel_map.items():
            if key in keys_observadas:
                continue

            centro = self.voxel_center(key)
            dist_sensor = np.linalg.norm(centro - sensor_pos)
            dist_ultima_vista = np.linalg.norm(sensor_pos - voxel["last_seen_pos"])

            if dist_sensor <= self.radio_actualizacion and dist_ultima_vista <= self.radio_cercania_vista:
                voxel["score"] -= self.miss_penalty

                if voxel["score"] < self.score_delete_threshold:
                    keys_borrar.append(key)
    ...
    #######
    def actualizar_voxel_map(self, cloud, sensor_pos):
        keys_observadas = self.anadir_hits_voxel_map(cloud, sensor_pos)
        self.penalizar_misses_voxel_map(keys_observadas, sensor_pos)
    #######


    #######
    def voxel_map_to_cloud(self):
        pts = []
        cols = []

        for voxel in self.voxel_map.values():
            if voxel["score"] < self.score_min_publish:
                continue

            p = voxel["sum_xyz"] / voxel["count_hits"]
            c = voxel["sum_rgb"] / voxel["count_hits"]
        ...
        if len(pts) > 0:
            cloud.points = o3d.utility.Vector3dVector(np.array(pts, dtype=np.float64))
            cloud.colors = o3d.utility.Vector3dVector(np.array(cols, dtype=np.float64))
    ...
    def fusionar_en_mapa(self, source_raw, target_local, T_final):
        ...
        #######
        sensor_pos = T_final[:3, 3].copy()
        self.actualizar_voxel_map(nueva_pc, sensor_pos)
        #######
    ...
    def cloud_cb(self, cloud_msg):
        ...
        if self.latest_pose is not None:
            ...
            if len(self.global_cloud.points) == 0:
                ...
                #######
                self.actualizar_voxel_map(self.global_cloud, T_cuerpo_mapa[:3, 3].copy())
                #######
            else:
                ...
                if result is not None:
                    ...
                    #######
                    # Si ICP falla, NO se añaden hits de esta nube al mapa probabilístico.
                    # Así evitamos reforzar una nube errónea.
                    #######
            ...
            #######
            # La representación publicada pasa a salir del mapa probabilístico por vóxeles
            self.global_cloud = self.voxel_map_to_cloud()
            #######
