...
public:
  Clase_Subscriber()
  : rclcpp::Node("graficar_GTvsTray")
  {
    objeto_subscriber_pose = this->create_subscription<geometry_msgs::msg::PoseStamped>(
      "sensor/GT/pose", 10,
      std::bind(&Clase_Subscriber::pose_callback, this, std::placeholders::_1));
    ...
    objeto_subscriber_feedback =
      this->create_subscription<dron_individual::action::TrayAction_FeedbackMessage>(
      "AccionTrayectoria/_action/feedback", 10,
      std::bind(&Clase_Subscriber::feedback_callback, this, std::placeholders::_1));
    ...
    objeto_timer =
      this->create_wall_timer(20ms, std::bind(&Clase_Subscriber::enviar_graficas, this));
  }
...
private:
  ...
  {
    valor_real.data =
    {msg_pose->pose.position.x, msg_pose->pose.position.y, msg_pose->pose.position.z, pose2yaw(
        *msg_pose)};
  ...
  {
    valor_real.data =
    {msg_acc->accel.linear.x, msg_acc->accel.linear.y, msg_acc->accel.linear.z,
      msg_acc->accel.angular.z};

    labels_real = {"ax_real", "ay_real", "az_real", "ayaw_real"};
  }

  void feedback_callback(const dron_individual::action::TrayAction_FeedbackMessage::SharedPtr msg)
  {
  ...
  void enviar_graficas()
  {
    if (tray_activada) {
      ...
      std_msgs::msg::Float64MultiArray enviar;
      enviar.data =
      {valor_tray.data[0] - valor_real.data[0], valor_tray.data[1] - valor_real.data[1],
        valor_tray.data[2] - valor_real.data[2], valor_tray.data[3] - valor_real.data[3]};
      objeto_publisher_num->publish(enviar);
