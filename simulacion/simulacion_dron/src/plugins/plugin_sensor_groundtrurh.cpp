...
public:
  void Load(physics::ModelPtr model, sdf::ElementPtr sdf) override
  {
    ...
    if (!link_) {
      RCLCPP_ERROR(
        ros_node_->get_logger(), "PluginSensorGroundtruth: no encuentro link %s",
        link_name_.c_str());
    } else {
      pub_pose_ =
        ros_node_->create_publisher<geometry_msgs::msg::PoseStamped>(topic_pose_name_, 10);
    ...
    update_conn_ =
      ...
      std::bind(
        &PluginSensorGroundtruth::OnUpdate, this,
        std::placeholders::_1));
  }
